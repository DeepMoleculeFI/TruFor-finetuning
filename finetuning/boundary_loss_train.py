...
Continues training from a pretrained model checkpoint with Boundary Loss
"""
...
def boundary_loss(pred, mask, smooth=1.0):
    """
    Calculate boundary loss to improve segmentation at edges
    
    Args:
        pred: Prediction tensor (B, H, W)
    ...
    Returns:
        Boundary loss value
    """
    # Apply sigmoid to prediction if it's logits
    if pred.max() > 1 or pred.min() < 0:
        pred = torch.sigmoid(pred)
    ...
    # Extract edges using average pooling difference
    pred_edge = pred - F.avg_pool2d(pred, kernel_size=3, stride=1, padding=1)
    mask_edge = mask - F.avg_pool2d(mask, kernel_size=3, stride=1, padding=1)
    
    # Calculate intersection and union at boundaries
    intersection = (pred_edge * mask_edge).sum()
    union = pred_edge.sum() + mask_edge.sum()
    
    # Return boundary loss
    return 1 - (2 * intersection + smooth) / (union + smooth)

def train(model, train_loader, optimizer, device, epoch, num_epochs, boundary_weight=0.5):
    """
    ...
    for images, masks in progress_bar:
        ...
        bl = boundary_loss(outputs[:, 1], masks.float())
        
        # Combined loss
        loss = dl + boundary_weight * bl
        
        loss.backward()
        optimizer.step()
        
        train_loss += loss.item()
        progress_bar.set_postfix({'loss': loss.item(), 'dice_loss': dl.item(), 'boundary_loss': bl.item()})
    
    return train_loss / len(train_loader)

def evaluate(model, val_loader, device, threshold=0.5, boundary_weight=0.5):
    """
...
def parse_args():
    parser = argparse.ArgumentParser(description='Fine-tune AI Manipulation Detection model with Boundary Loss')
...
def main():
    ...
    val_loss, dice_score = evaluate(model, val_loader, device, args.threshold, args.boundary_weight)
    ...
    for epoch in range(1, args.epochs + 1):
        # Train for one epoch
        train_loss = train(model, train_loader, optimizer, device, epoch, args.epochs, args.boundary_weight)
    ...
    wandb.log({
        ...
        'dice_improvement': best_dice - initial_dice  # Improvement from initial score
    })
    wandb.finish()
